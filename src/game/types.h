 ...
 *
 * CR?TICO -- m68k / SGDK 1.70:
 *   int  = 16 bits  -> NUNCA usar int para valores monet?rios ou contagens > 32767
 *   long = 32 bits  -> usar para sal?rios, receitas, transfer?ncias
 *   sizeof(int) = 2, sizeof(long) = 4, sizeof(void*) = 4
 *
 * As estruturas PlayerRecord / TeamRecord / CoachRecord definem o layout
 * BIN?RIO de teams.bin e coaches.bin gerados por pack_data.py.
 * Qualquer altera??o aqui exige regenerar os bin?rios.
 *
 * PlayerRecord e TeamRecord usam padding expl?cito (_pad) para garantir
 * alinhamento de word no m68k sem depender de padding impl?cito do GCC,
 * que varia com flags de compila??o.
 */
...
#define MIN_SQUAD_SIZE     14u   /* m?nimo de jogadores no plantel      */
#define MIN_GK_COUNT        1u   /* m?nimo de guarda-redes no plantel   */
...
/*
 * Position -- posi??o de campo do jogador.
...
/*
 * Formation -- ?ndice de forma??o t?tica (0-9).
 * O nome e o array de multiplicadores est?o em game/data.c.
 */
typedef enum {
    FORM_4_4_2  = 0,
    FORM_4_3_3  = 1,
...
/*
 * GameScreen -- estados da m?quina de telas.
 */
typedef enum {
    SCREEN_TITLE,
    SCREEN_MAIN_MENU,
...
/* ------------------------------------------------------------------ */
/* Estruturas bin?rias (layout de ROM -- teams.bin / coaches.bin)      */
/* ------------------------------------------------------------------ */
...
/*
 * Player -- estado em runtime de um jogador (em RAM, n?o em ROM).
...
typedef struct {
    ...
    u8    n2;                 /* fator de or?amento/for?a inicial     */
    u8    division;           /* 0=Div1 ... 3=Div4                      */
    u8    formation;          /* Formation enum (0-9)                  */
    u8    coach_idx;          /* ?ndice em g_coaches[] (0-49)         */
    u8    player_start;       /* ?ndice do 1? jogador em g_players[]  */
    ...
    /* Finan?as (32 bits -- podem exceder 32767 facilmente)            */
    long  money;
    long  salary_total;
...
/*
 * CupTie -- confronto de copa em 2 m?os.
 */
typedef struct {
    u8  team_a;
    u8  team_b;
    ...
    u8  goals_a_leg2;
    u8  goals_b_leg2;
    u8  phase;     /* 0=eliminat?rias, 1=quartos, 2=meias, 3=final    */
    u8  _pad;
} CupTie;
...
typedef struct {
    u8  season_num;
    u8  div1_champion;   /* ?ndice da equipe campe? da Div1            */
    u8  cup_champion;    /* ?ndice da equipe campe? da copa            */
    u8  player_div;      /* divis?o do jogador nessa temporada         */
    u8  player_pos;      /* posi??o final do jogador na classifica??o  */
    u8  _pad[3];
} SeasonRecord;

#define PALMARES_COUNT  5u   /* m?ximo de temporadas no palmares       */

/* ------------------------------------------------------------------ */
/* Macros utilit?rias                                                  */
/* ------------------------------------------------------------------ */

/* Calcula pontos de classifica??o a partir de wins/draws.             */
