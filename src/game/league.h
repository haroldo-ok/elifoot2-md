...
/*
 * game/league.h — Campeonato: calendarização de jogos e classificação
 *
 * O campeonato usa um sistema de todos-contra-todos (round-robin) dentro
 * de cada divisão. Com 16 equipas na Div1, são 15 jornadas de 8 jogos
 * cada (ida) ou 30 jornadas (ida e volta). Para simplificar a RAM,
 * implementamos apenas a volta simples (15 jornadas).
 *
 * Com 13 equipas na Div2, são 13 jornadas (algoritmo de round-robin
 * com bye para número ímpar de equipas).
 */
...
/*
 * LeagueRound — jogos de uma jornada de uma divisão.
 * home[i] e away[i] são índices em g_teams[].
 * count = número de jogos (até 8).
 */
typedef struct {
...
/*
 * league_build_round() — Constrói os pares de jogos para a jornada
 * round_num (1-based) da divisão div, usando algoritmo de round-robin
 ...
 * NÃO chama SYS_doVBlankProcess() — é seguro chamar em loop.
 */
void league_simulate_round(const LeagueRound *round);

/*
 * league_total_rounds() — Número total de jornadas para a divisão div.
 */
