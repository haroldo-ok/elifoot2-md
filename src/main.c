/*
 * main.c -- Entrada principal do Elifoot II Genesis (versao completa)
 */
...
#include "screens/alterar_ord.h"
#include "screens/marcadores.h"
#include "screens/proximas.h"
#include "screens/resultados.h"
#include "screens/calendario.h"
#include "screens/ultimas_transf.h"
...
static u8 startup_screen(void) {
    ...
    for (;;) {
        ...
        { u8 s; for (s=0u; s<3u; s++) {
            if (sram_slot_valid(s))
                ui_printf(8u,(u16)(18u+s),UI_PAL_NORMAL,"Slot %u: Temp.%u",(u16)(s+1u),(u16)sram_slot_season(s));
            else
                ui_printf(8u,(u16)(18u+s),UI_PAL_NORMAL,"Slot %u: (vazio)",(u16)(s+1u)); } }
...
int main(bool hardReset) {
    ...
    { u8 choice = startup_screen();
      if (choice == 1u) {
          ...
          for (s=0u; s<3u && !loaded; s++)
              if (sram_slot_valid(s)) loaded = sram_load(s);
          if (!loaded) goto new_game;
      } else {
          new_game:
    ...
    for (;;) {
        ...
        switch (choice) {
            ...
            case MENU_RESULT_MARCADORES: screen_marcadores();   break;
            case MENU_RESULT_PROXIMAS:   screen_proximas();     break;
            case MENU_RESULT_RESULTADOS: screen_resultados();   break;
            case MENU_RESULT_CALENDARIO: screen_calendario();    break;
            case MENU_RESULT_ULTIMAS_T:  screen_ultimas_transf(); break;
            default: break;
        }
    }
    return 0;
